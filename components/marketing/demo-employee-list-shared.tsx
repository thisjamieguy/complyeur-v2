...
export interface DemoEmployeeListRowsProps {
  rows: DemoEmployee[]
...
export function getAutonomousEmployees(frameIndex = 0): DemoEmployee[] {
  return employeeAnimations.map((employee, index) => {
    const frame = employee.frames[frameIndex]
    return {
      id: index,
      name: employee.name,
      lastTrip: employee.lastTrip,
...
export function getAutonomousFrameCount(): number {
  return employeeAnimations[0]?.frames.length ?? 0
}

export function DemoEmployeeListRows({
  rows,
  highlightedEmployeeName,
  animated = false,
}: DemoEmployeeListRowsProps) {
              ...
              return (
                <tr
                  ...
                  className={cn(
                    ...
                    isHighlighted &&
                      'bg-brand-50/60 shadow-[inset_0_0_0_1px_rgba(53,92,130,0.22)]',
                    animated && isHighlighted && 'motion-safe:animate-pulse'
                  )}
                >
                  ...
                  <td className="px-4 py-3">
                    <DemoStatusBadge status={employee.status} animated={animated} />
                  </td>
                  <td className="hidden px-4 py-3 sm:table-cell">
                    <div className="flex items-center gap-2">
                      <span
                        className={cn(
                          'tabular-nums text-sm text-slate-600',
                          animated && 'transition-all duration-500'
                        )}
                      >
