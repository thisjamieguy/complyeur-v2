 ...
 * - dsar_manifest.json: Machine-readable included/excluded store manifest
 */
...
type DsarManifestStatus =
  | 'included_in_zip'
  | 'manual_account_process'
  | 'processor_or_external_system'
  | 'retained_with_documented_limitation'

interface DsarManifestEntry {
  store: string
  status: DsarManifestStatus
  files: string[]
  fields: string[]
...
interface ExportMetadata {
  ...
  dsar_manifest: DsarManifestEntry[]
  retention_policy: {
    months: number
...
- dsar_manifest.json: Machine-readable list of included stores and documented exclusions
...
export async function generateDsarExport(
  ...
  try {
    ...
    const dsarManifest: DsarManifestEntry[] = [
      {
        store: 'employees',
        ...
        record_count: 1,
        rationale: 'Primary employee record for the data subject.',
      },
      {
        store: 'trips',
        status: 'included_in_zip',
        ...
        record_count: tripData.length,
        rationale: 'Employee-linked travel history used for Schengen compliance calculations.',
      },
      {
        store: 'alerts',
        status: 'included_in_zip',
        ...
        record_count: alertData.length,
        rationale: 'Employee-linked compliance alert records.',
      },
      {
        ...
        record_count: notificationLogData.length,
        rationale: 'Employee-linked notification delivery records retained by the application.',
      },
      {
        ...
        record_count: importSessionData.length,
        rationale: 'Retained import staging/history rows matched to the data subject where not yet purged by retention.',
      },
      {
        ...
        record_count: storedComplianceSnapshotData.length,
        rationale: 'Stored employee-linked compliance snapshots generated by the application.',
      },
      {
        store: 'generated_current_compliance_calculation',
        status: 'included_in_zip',
        files: ['current_compliance_calculation.json'],
        fields: Object.keys(complianceSnapshot),
        record_count: 1,
        rationale: 'Generated at export time from current trip data; not a separately stored historical table.',
      },
      {
        store: 'profiles',
        ...
        record_count: null,
        rationale: 'Account-user data is handled through the account-level DSR process rather than the employee-level export.',
      },
      {
        ...
        record_count: null,
        rationale: 'User preference data is handled through the account-level DSR process.',
      },
      {
        ...
        fields: [
          'user_id',
          'email/contact details where supplied',
          'message',
          'page path',
          'timestamps',
        ],
        record_count: null,
        rationale: 'Support and feedback content may identify account users and is handled through the account-level DSR process.',
      },
      {
        store: 'billing_and_stripe_metadata',
        status: 'processor_or_external_system',
        files: [],
        fields: ['stripe_customer_id', 'subscription metadata', 'invoice/payment records'],
        record_count: null,
        rationale: 'Billing records are processor-managed and subject to tax/accounting retention; exported manually when applicable.',
      },
      {
        store: 'supabase_auth',
        status: 'processor_or_external_system',
        files: [],
        fields: ['auth user id', 'email', 'provider metadata', 'session/security metadata'],
        record_count: null,
        rationale: 'Authentication records are managed through Supabase Auth and handled in account-level DSR workflows.',
      },
      {
        store: 'application_logs_and_sentry',
        status: 'processor_or_external_system',
        files: [],
        fields: ['event metadata', 'request context', 'user identifiers where present'],
        record_count: null,
        rationale: 'Operational logs are minimized and searched manually where technically feasible for account-level requests.',
      },
      {
        store: 'dsar_export_archives',
        status: 'retained_with_documented_limitation',
        files: [],
        fields: ['generated ZIP archive', 'signed URL metadata'],
        record_count: null,
        rationale: 'Generated archives are temporary private storage objects and purged by retention cleanup.',
      },
      {
        store: 'backups',
        status: 'retained_with_documented_limitation',
        files: [],
        fields: ['database backup snapshots'],
        record_count: null,
        rationale: 'Backups are restore-only disaster recovery copies that expire according to the documented backup window.',
      },
    ]
    ...
    zip.file('dsar_manifest.json', JSON.stringify(dsarManifest, null, 2))
